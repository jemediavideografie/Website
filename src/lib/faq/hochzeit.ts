import type { FaqItem } from "../content";

export const hochzeit: FaqItem[] = [
  {
    question: "Wie kamst du zur Hochzeitsvidegrafie?",
    answer:
      "Ich wurde angefragt die Hochzeit einer befreundeten Familie zu filmen. Ich war mit der Kamera dabei, eher aus Ahnung wie eine Kamera benutze als aus Plan. Doch als ich das fertige Video zum ersten Mal abspielte und ich, die Reaktion des Brautpaares bekam wusste ich sofort: Das will ich machen. Nicht als großes Business. Als Handwerk, das etwas hinterlässt.",
  },
  {
    question: "Wie läuft die Buchung ab?",
    answer:
      "Alles fängt mit eurer Buchungsanfrage an. Falls euer Tag noch in meinem Kalender frei ist, setzen wir uns via Skype oder Telefon zusammen und sprechen alles durch.",
  },
  {
    question: "Welche Momente werden gefilmt?",
    answer:
      "Ich begleite euren Tag mit einem Blick für echte Emotionen und besondere Augenblicke - vom Getting Ready oder First Look über die Trauung bis hin zur Feier. So wird eure Geschichte authentisch und emotional festgehalten.",
  },
  {
    question: "Arbeitest du alleine oder im Team?",
    answer:
      "Ich arbeite in der Regel alleine. Dadurch kann ich flexibel, unauffällig und persönlich auf euren Tag eingehen.",
  },
  {
    question: "Was passiert bei schlechtem Wetter?",
    answer:
      "Auch bei schlechtem Wetter entstehen einzigartige und emotionale Aufnahmen. Regen oder bewölkter Himmel sorgen oft sogar für eine besondere Stimmung im Video. Ich passe mich flexibel an die Gegebenheiten an und finde immer kreative Lösungen.",
  },
  {
    question: "Wie lang ist das fertige Hochzeitsvideo?",
    answer:
      "Die Länge eures Films hängt vom gebuchten Paket ab. In der Regel erhaltet ihr einen hochwertigen Highlightfilm, der die schönsten und emotionalsten Momente eures Tages kompakt und cinematisch zusammenfasst. Auf Wunsch sind auch längere Versionen oder zusätzliche Inhalte möglich.",
  },
  {
    question: "In welcher Qualität erhalten wir das Video?",
    answer: "Ihr erhaltet euer fertiges Hochzeitsvideo in 4K-Qualität als Download.",
  },
  {
    question: "Verwendest du Drohnenaufnahmen?",
    answer:
      "Drohnenaufnahmen sind möglich. Für besondere Anlässe wie Hochzeiten oder Events wird die Drohne bei Bedarf organisiert und eingesetzt.",
  },
  {
    question: "Wie würdest du deinen Videostil beschreiben?",
    answer:
      "Mein Fokus liegt auf emotionalen, natürlichen und cinematischen Aufnahmen. Ich möchte echte Momente einfangen und daraus einen Film erstellen, der euch auch Jahre später wieder in euren Tag zurückversetzt.",
  },
  {
    question: "Welches Equipment nutzt du?",
    answer:
      "Ich arbeite mit professionellem Equipment, darunter eine Sony A7 III in Kombination mit einem stabilisierten Gimbal (RS4 Pro). Dadurch entstehen ruhige, hochwertige und kinoreife Aufnahmen. Als Allround-Objektiv nutze ich ein 24-70mm, um flexibel auf jede Situation reagieren zu können.",
  },
  {
    question: "Gibt es verschiedene Pakete?",
    answer:
      "Ja, es gibt verschiedene Pakete - von kurzen Begleitungen bis hin zur ganztägigen Hochzeitsreportage. Die genauen Inhalte und Preise findet ihr in der Übersicht oder erhaltet sie individuell auf Anfrage.",
  },
  {
    question: "Kann man Leistungen individuell anpassen?",
    answer:
      "Ja, alle Leistungen können individuell auf eure Wünsche abgestimmt werden. Gemeinsam finden wir die passende Lösung für euren Tag.",
  },
  {
    question: "Ist eine Anzahlung erforderlich?",
    answer:
      "Ja. Zur festen Reservierung eures Termins ist eine Anzahlung erforderlich. Hierzu erhaltet ihr eine Anzahlungsrechnung. Erst nach Zahlungseingang gilt euer Termin als verbindlich gebucht.",
  },
  {
    question: "Wie lange dauert es, bis der Film fertig ist?",
    answer:
      "Ich verstehe gut, dass ihr es kaum erwarten könnt, euren Film Familie und Freunden zu zeigen! Ich rechne meistens mit einer Bearbeitungszeit von 6–8 Wochen und dann noch zwei weiteren Wochen für eventuelle Korrekturen. In Einzelfällen kann es ein wenig länger dauern.",
  },
  {
    question: "Wie funktioniert das mit den Änderungen?",
    answer:
      "Sobald ich mit der Nachbereitung eures Films fertig bin, erhaltet ihr einen Link, unter dem ihr den Film anschauen könnt. Ihr könnt euch dann etwas Zeit nehmen und mir eure Änderungswünsche gesammelt zuschicken. Ich bearbeite diese dann und schicke euch eine neue Version des Films zu.",
  },
  {
    question: "Wie steht's mit der Verpflegung?",
    answer:
      "Macht euch um mich keine Sorgen – aber zu einer Einladung ans Buffet oder was auch immer euch vorschwebt, würde ich keinesfalls nein sagen 😊 Platziert mich, wo ihr wollt – ein kleiner Plausch mit euren Gästen ist mir jederzeit eine Freude!",
  },
  {
    question: "Bietest du eine Absicherung bei unvorhergesehenen Ereignissen an?",
    answer:
      "Klar – sollte euer Tag durch unvorhersehbare Einschränkungen nicht stattfinden können, versuchen wir gemeinsam eine Lösung zu finden. Sprecht mich gerne darauf an!",
  },
];
